-- Combined search RPC with random sort support and product images.
-- Adds p_seed for deterministic random ordering so marketplace discovery works.
...
BEGIN
  ...
  sorted AS (
    ...
    ORDER BY
      CASE WHEN p_sort = 'random' THEN md5(p_seed::text || ':' || id::text) END,
      CASE p_sort
        WHEN 'newest' THEN 0
...
GRANT EXECUTE ON FUNCTION public.search_products_combined(
  text, uuid, text, numeric, numeric, numeric, boolean, text, text, integer, real, timestamptz, uuid, uuid
