...
- **Variant image swap: IMPLEMENTED** ✓ — `product_variants.image_url` is selected by the PDP hook; when a variant with its own image is selected, that image is shown first in the gallery (AliExpress behavior). Seller form Variants tab now has an **Image upload** control per variant row (JPEG/PNG/WebP file picker with live thumbnail preview), files are uploaded to storage then saved to `image_url` on insert, and loaded back on edit.
- **Infinite "You May Also Like"** ✓ — Recommended products grid uses `InfiniteScrollTrigger` with `range`-based pagination (12 per page) so more products keep loading as the user scrolls — the product page never ends, matching AliExpress's endless discovery feed.
