...
export default function QAndASection({ productId }: { productId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    if (list.length === 0) { setQuestions([]); setLoading(false); return; }
  ...
  return (
    <section id="qa" className="mt-6 border-t border-[#E8E8E8] pt-6 dark:border-[#222222]">
        ...
        <div className="space-y-4">
          ...
          {questions.length > 2 && (
            <button
              ...
              className="min-h-11 text-xs font-bold text-[#111111] underline-offset-4 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#111111] focus-visible:ring-offset-2 dark:text-[#FAF5F2] dark:focus-visible:ring-[#FAF5F2]"
            >
              {showAllQuestions ? "Show fewer" : "See all questions"}
            </button>
          )}
        </div>
      )}
